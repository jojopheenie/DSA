...
Instantiate a new ‘max’ array to hold the max gold that can be accumulated up to each house.
Loop through the input array with, i.
For each house, the max gold (max[i]) will be the greater of:
The previous house’s max
The max two houses back + the current house value
After the loop, return the last value in the max array
let test1 = [1, 2, 3];
